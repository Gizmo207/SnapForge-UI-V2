import AnimatedPattern from "./components/vault/AnimatedPattern";
import { KineticText } from "./components/vault/KineticText";
import AnimatedButton from "./components/vault/AnimatedButton";
import GradientCard from "./components/vault/GradientCard";
import Card3D from "./components/vault/Card3D";
import AnimatedToggle from "./components/vault/AnimatedToggle";
import AnimatedLoader from "./components/vault/AnimatedLoader";
import LiquidLoader from "./components/vault/LiquidLoader";
import GamepadCheckbox from "./components/vault/GamepadCheckbox";
import { Marquee } from "./components/vault/Marquee";
import GlassSurface from "./components/vault/GlassSurface";
import TwoPiWave from "./components/vault/TwoPiWave";
import LightfallNeon from "./components/vault/LightfallNeon";
...
const INGEST = [
  ...
  },
  {
    ...
    iconPath:
      "M4 4h16v4H4V4zm0 6h16v4H4v-4zm0 6h10v4H4v-4zM8 5.5L6 7.5 8 9.5M11 5.5h3",
  },
  {
    title: "Folder upload",
    description:
...
const MCP_TOOLS = [
  {
    name: "search_vault",
    desc: "Keyword search across your saved components by name, tag, or category.",
  },
  {
    name: "list_vault",
    desc: "List everything in the library — optionally filtered to one category.",
  },
  {
    name: "get_component",
    desc: "Return export-ready source, deps, multi-file map, CSS, and demo.",
  },
];
    ...
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      <header className="fixed inset-x-0 top-0 z-50 flex justify-center px-4 pt-4">
        <nav className="flex w-full max-w-6xl items-center justify-between rounded-2xl border border-white/10 bg-[#0a0a14]/70 px-5 py-3 backdrop-blur-xl">
          ...
          <AnimatedButton label="Start your vault" />
        </nav>
      </header>
      ...
      >
        {/* Vault: "Neon Pattern" / Lightfall — falling neon streaks */}
        ...
        {/* Scrim for headline legibility + fade into the next section */}
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-white/70 sm:text-xl">
            Save any React or HTML component to your own library — from uiverse,
            React Bits, Magic UI, shadcn, Aceternity, or your own code. SnapForge
            classifies it, previews it live, exports it, and hands it to your AI
            agent.{" "}
            <strong className="text-white">
              Any component you save, ready to ship.
            </strong>
          </p>
          ...
          <div className="mt-9 flex flex-wrap items-center justify-center gap-4">
            ...
            <a
              href="#mcp"
              className="rounded-xl border border-white/15 bg-white/5 px-5 py-3 text-sm font-semibold text-white/85 transition hover:border-white/30 hover:bg-white/10"
            >
              Connect your AI →
            </a>
          </div>
          ...
          <p className="mt-8 font-mono text-xs text-white/40">
            React &amp; HTML · live sandbox previews · MCP-native
          </p>
        </div>
      </section>
      ...
      <section className="relative z-10 space-y-3 border-y border-white/10 bg-[#08080f] py-6">
        <Marquee pauseOnHover className="[--duration:30s] [--gap:1.1rem]">
          {SOURCES.map((t) => (
            <div
              key={t}
              className="flex items-center whitespace-nowrap rounded-2xl border border-white/10 bg-gradient-to-br from-[#1c1c2e] to-[#13131f] px-6 py-3.5 text-base font-bold text-white/90"
            >
              {t}
            </div>
          ))}
        ...
        <Marquee reverse pauseOnHover className="[--duration:34s] [--gap:1.1rem]">
          {CAPS.map((t) => (
            <div
              key={t}
              className="flex items-center whitespace-nowrap rounded-2xl border border-violet-400/15 bg-gradient-to-br from-[#1a1430] to-[#130f22] px-6 py-3.5 text-base font-bold text-violet-100/90"
            >
              {t}
            </div>
          ))}
      ...
      <section
        id="how"
        className="relative z-10 mx-auto max-w-6xl px-6 py-28 text-center"
      >
        ...
        <p className="mx-auto mt-5 max-w-2xl text-white/60">
          Hover a card for the details. Paste, pull from a registry, or upload —
          every component is parsed, classified, security-gated, and stored under
          your account.
        </p>
      ...
      <section id="preview" className="relative z-10 border-y border-white/10">
        <div className="relative h-[520px] w-full overflow-hidden bg-[#070710]">
          {/* Vault: TWO PI dot-field as a light rainbow wave wall */}
          ...
          <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center">
            ...
            <h2 className="mt-6 max-w-3xl text-4xl font-bold tracking-tight sm:text-5xl">
              See every component render live
            </h2>
            <p className="mx-auto mt-5 max-w-2xl text-white/70">
              Each save bundles in a real in-browser sandbox — Tailwind, the
              shadcn <code className="text-violet-200">cn()</code> helper, Magic UI
              keyframes, styled-components, WebGL, glass &amp; backdrop-filter. No
              "looks right in the docs," then breaks in your app.{" "}
              <span className="text-white/90">
                Move your cursor across this wall — that's a vault component too.
              </span>
            </p>
          </div>
        </div>
      ...
      <section className="relative z-10 bg-[#07070d] py-28">
        <div className="mx-auto max-w-6xl px-6 text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-white/60">
            3D cards, physical toggles, molten loaders, glowing devices — every
            tile below is a real saved component, previewing live, exactly as it
            would in your project.
          </p>

          <div className="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            <ShowcaseCell label="3D Card · components/cards">
              <Card3D
                heading="Saved this month"
                line1="Lives in"
                line2="your vault"
              />
            </ShowcaseCell>
      ...
      <section
        id="mcp"
        className="relative z-10 border-y border-white/10 bg-[#08080f] py-28"
      >
        <div className="mx-auto max-w-6xl px-6">
          <div className="text-center">
            <SectionLabel>The differentiator</SectionLabel>
            <h2 className="mt-6 text-4xl font-bold tracking-tight sm:text-5xl">
              Hand your vault to your{" "}
              <span className="bg-gradient-to-r from-violet-400 to-fuchsia-400 bg-clip-text text-transparent">
                AI agent
              </span>
            </h2>
            <p className="mx-auto mt-5 max-w-2xl text-white/60">
              A remote MCP server connects Claude Code, Cursor, Windsurf, and VS
              Code straight to your library — read-only, behind a per-user token.
              Then just ask: <em className="text-white/85">"build a landing page
              using only components from my SnapForge vault."</em>
            </p>
          </div>
            ...
            {MCP_TOOLS.map((tool) => (
              ...
              >
                <div className="flex h-full w-full flex-col justify-center px-7 text-left">
                  <code className="text-lg font-bold text-violet-200">
                    {tool.name}
                  </code>
          ...
          <div className="mx-auto mt-10 max-w-3xl rounded-2xl border border-white/10 bg-black/60 p-5 font-mono text-sm leading-relaxed text-white/80">
            <div>
              <span className="select-none text-white/35">$ </span>claude mcp add
              snapforge --transport http \
            </div>
            ...
            <div className="mt-3 text-white/40">
              # tokens are owner-scoped — only the SHA-256 hash is ever stored
            </div>
          </div>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3 text-xs text-white/55">
            {["Claude Code", "Cursor", "Windsurf", "VS Code"].map((c) => (
              <span
                key={c}
                className="rounded-full border border-white/10 bg-white/5 px-4 py-1.5 font-semibold"
              >
                {c}
              </span>
            ))}
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-6 py-28">
        <div className="grid items-center gap-12 lg:grid-cols-2">
          <div>
            ...
            <h2 className="mt-6 text-4xl font-bold tracking-tight sm:text-5xl">
              Every paste runs the gate
            </h2>
            <p className="mt-5 max-w-xl text-white/60">
              Untrusted code from the internet never renders blind. A
              deterministic sanitization gate inspects each component and tags the
              outcome <code className="text-emerald-300">allowed</code>,{" "}
              <code className="text-rose-300">blocked</code>, or{" "}
              <code className="text-amber-300">invalid</code> before it ever
              previews.
            </p>
            <p className="mt-4 max-w-xl text-sm text-white/45">
              Multi-tenant by design: row-level security, every component scoped
              to your <code>owner_id</code>. Your vault is yours alone.
            </p>
          </div>

          <div className="rounded-3xl border border-white/10 bg-[#0c0c16] p-8">
            <p className="mb-5 text-xs font-semibold uppercase tracking-[0.2em] text-rose-300/80">
              Blocked on sight
            </p>
            <ul className="space-y-3">
              {BLOCKED.map((b) => (
                <li
                  key={b}
                  className="flex items-center gap-3 rounded-xl border border-white/5 bg-black/40 px-4 py-3 font-mono text-sm text-white/80"
                >
      ...
      <section
        id="pricing"
        className="relative z-10 border-y border-white/10 bg-[#07070d] py-28"
      >
        <div className="mx-auto max-w-6xl px-6 text-center">
          <SectionLabel>Plans</SectionLabel>
          <h2 className="mt-6 text-4xl font-bold tracking-tight sm:text-5xl">
            Start free. Forge faster when you're ready.
          </h2>
            ...
            {[
              {
                ...
                feats: ["Unlimited saves", "Live previews", "ZIP export"],
                highlight: false,
              },
              {
                name: "Pro",
              ...
              },
              {
                ...
                feats: ["Everything in Pro", "Shared libraries", "Seat controls"],
                highlight: false,
              },
            ].map((p) => (
              <GlassSurface
                ...
                height={320}
                borderRadius={26}
                backgroundOpacity={p.highlight ? 0.06 : 0}
                className="w-full"
              >
                <div className="flex h-full w-full flex-col items-center justify-center px-8 text-center">
                  {p.highlight && (
                    <span className="mb-3 rounded-full bg-gradient-to-r from-violet-500 to-fuchsia-500 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white">
                      Most popular
                    </span>
                  )}
          ...
          <p className="mt-8 text-xs text-white/40">
            Pricing in progress — MCP access ships as a Pro feature.
          </p>
        </div>
      </section>
      ...
      <section
        id="start"
        className="relative z-10 overflow-hidden px-6 py-28 text-center"
      >
        {/* Vault: "Animated Pattern" — kana matrix */}
        ...
        <div className="relative z-10 mx-auto flex max-w-3xl flex-col items-center">
          <KineticText
            text="Forge faster."
            className="justify-center text-5xl font-light tracking-tight drop-shadow-[0_2px_20px_rgba(0,0,0,0.6)] sm:text-6xl"
          />
          <p className="mt-6 max-w-xl text-white/80">
            Build the library once. Preview everything live. Then let your AI
            agent pull from it on demand — no more rebuilding the same component
            twice.
          </p>
          ...
          <div className="mt-9 flex flex-wrap items-center justify-center gap-4">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-xl border border-white/20 bg-black/30 px-5 py-3 text-sm font-semibold text-white/90 backdrop-blur-sm transition hover:border-white/40 hover:bg-black/50"
            >
        ...
        <footer className="relative z-10 mx-auto mt-32 flex max-w-6xl flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 text-sm text-white/55 sm:flex-row">
          ...
          <span className="font-mono">
            Built entirely from vault components ✦
          </span>
        </footer>
      </section>
    </div>
